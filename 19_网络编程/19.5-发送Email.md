...
MUA到MTA发送邮件的协议就是SMTP协议，它是`Simple Mail Transport Protocol`的缩写，使用标准端口25，也可以使用加密端口465或587。
...
![javamail-text](pic/19.5-%E5%8F%91%E9%80%81Email/1.png)
...
![javamail-html](pic/19.5-%E5%8F%91%E9%80%81Email/2.png)
...
![javamail-attachment](pic/19.5-%E5%8F%91%E9%80%81Email/3.png)
...
在HTML邮件中引用图片时，需要设定一个ID，用类似`<img src=\"cid:img01\">`引用，然后，在添加图片作为`BodyPart`时，除了要正确设置MIME类型（根据图片类型使用`image/jpeg`或`image/png`），还需要设置一个Header：
...
![javamail-inline](pic/19.5-%E5%8F%91%E9%80%81Email/4.png)
...
下载练习：[使用SMTP发送邮件](https://gitee.com/liaoxuefeng/learn-java/blob/master/practices/Java教程/150.网络编程.1255945371526048/40.发送Email.1319099923693601/network-smtp.zip?utm_source=blog_lxf)
