...
从继承关系可知：`Throwable`是异常体系的根，它继承自Object。`Throwable`有两个体系：`Error`和`Exception`，`Error`表示严重的错误，程序对此一般无能为力，错误也是不要求强制捕获的，例如：
...
如果是测试代码，上面的写法就略显麻烦。如果不想写任何try代码，可以直接把`main()`方法定义为`throws Exception`：
