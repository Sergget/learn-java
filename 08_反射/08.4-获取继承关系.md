...
运行上述代码，可以看到，`Integer`的父类类型是`Number`，`Number`的父类是`Object`，`Object`的父类是`null`。除`Object`外，其他任何非`interface`的Class都必定存在一个父类类型。

## 8.4.2 获取interface
由于一个类可能实现一个或多个接口，通过Class我们就可以查询到实现的接口类型。例如，查询`Integer`实现的接口：
...
此外，对所有`interface`的Class调用`getSuperclass(`)返回的是`null`，获取接口的父接口要用`getInterfaces()`：
