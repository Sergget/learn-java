...
public class DemoException {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(de.method()); //3
    }
}
```

首先要记住一点 ： `finally` 中的代码 一定会执行 先执行`try`中代码，如果`try`中代码没有异常且有返回值 ，先会执行 `finally`中代码 ，如果`finally`没有返回值，会回过头执行`try`的return;如果`finally`中有返回值。则直接返回。不会执行`try`中的`return`了； `catch` 同理。
